...
nínvel são os operadores de disjunção ($\lor$) e o segundo nível as
forlhas, compostas por lietrais.
...
#+INCLUDE: "~/unb/teaching/pspd/io-exemplos/input/simples" src
...
A saída é composta por diversas linhas podendo ser:
 ...
 - lista de literais falsos, sem repetição e ordenados por quantidade
   ...
   - Desempate (caso a ocorrência em clásulas tenha a mesma contagem)
     utilizando um algoritmo de ordenação estável.

Exemplo de saída para o exemplo acima:

#+INCLUDE: "~/unb/teaching/pspd/io-exemplos/output/simples" src
...
podem ser baixados [[./io-exemplos.tar.zst][AQUI]]
 - O arquivo está compactado com o algoritmo =zstd=, a maioria das
   distribuições GNU/Linux possuem o pacote =zstd=.
